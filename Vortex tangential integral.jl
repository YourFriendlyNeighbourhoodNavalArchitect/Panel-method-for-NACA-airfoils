begin
	function vortexTangentialIntegral(controlPanel, sourcePanel)
		...
		D = (xControlPoint - xStart)*sin(tangentControl) - (yControlPoint - yStart)*cos(tangentControl);
		if B - A^2 >= 0
			E = sqrt(B - A^2);
		else
			E = 0;
		...
		if (isnan(C/2*log((lengthSource^2 + 2*A*lengthSource + B)/B) + (D - A*C)/E*(dummyOne - dummyTwo)) == false) && (isinf(C/2*log((lengthSource^2 + 2*A*lengthSource + B)/B) + (D - A*C)/E*(dummyOne - dummyTwo)) == false) && ((lengthSource^2 + 2*A*lengthSource + B)/B > 0)
			N = C/2*log((lengthSource^2 + 2*A*lengthSource + B)/B) + (D - A*C)/E*(dummyOne - dummyTwo);
		else
			N = 0;
		end
		return N
